-- The account row's "Ẩn" (archive) action was the only way to remove a
-- cash/bank/savings account from view, which then landed it in a separate
-- "Tài khoản đã ẩn" section with a restore button — reported as unwanted
-- indirection ("đã xóa là xóa, còn ẩn gì"). Adding a real delete_account
-- action, matching the exact same pattern already used by
-- taichinh_gd_debt_ledger_api's 'delete': hard-delete when there is no
-- adjustment history to lose, otherwise refuse with a clear error instead
-- of silently archiving. archive_account/unarchive_account stay in the
-- function (harmless, no longer called from the UI) — the row's 🗑 button
-- now calls delete_account instead, and the "Tài khoản đã ẩn" section is
-- removed from the frontend in the same pass.
--
-- This reissues the whole taichinh_gd_api function (the only way to add a
-- branch to a plpgsql function body) — the body below is copied verbatim
-- from the live definition (pg_get_functiondef) with exactly one addition:
-- the new 'delete_account' branch right after 'unarchive_account'.
...
begin
  ...
  if p_action = 'health' then
    ...
    v_id := nullif(p_payload->>'id','')::uuid;
    if not exists(select 1 from taichinh_gd.accounts where id=v_id and household_id=h) then
      raise exception 'account_not_found';
    end if;
    if exists(select 1 from taichinh_gd.account_adjustments where account_id=v_id and household_id=h and deleted_at is null) then
      raise exception 'account_has_history';
    end if;
    delete from taichinh_gd.accounts where id=v_id and household_id=h;
