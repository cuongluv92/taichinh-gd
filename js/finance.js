...
function trendSvg(keys = Array.from({ length: 12 }, (_, i) => addMonths(state.month, i - 11))) {
  ...
  const bars = data.map((x, i) => {
    ...
    return `<g class="bar-group" ${act('showMonthBarAmounts', x.k)}><title>${x.k}: Thu ${money(x.inc)} · Chi ${money(x.exp)}</title><rect class="bar-income" x="${cx - bw - 2}" y="${base - ih}" width="${bw}" height="${ih}" rx="3"/><rect class="bar-expense" x="${cx + 2}" y="${base - eh}" width="${bw}" height="${eh}" rx="3"/><text class="axis-label trend-month-label" x="${cx}" y="${base + 15}" text-anchor="middle">${x.k.slice(5)}</text><text class="bar-value-line income" x="${cx}" y="${base + 31}" text-anchor="middle">${esc(compactMoney(x.inc))}</text><text class="bar-value-line expense" x="${cx}" y="${base + 46}" text-anchor="middle">${esc(compactMoney(x.exp))}</text></g>`;
  }).join('');
