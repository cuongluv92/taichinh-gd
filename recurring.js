(() => {
  ...
  if(typeof previousLoadMonth==='function')window.loadMonth=async function(m,rerender=true){
    await previousLoadMonth(m,false);
    await loadRecurringMonth(m,false);
    if(rerender)render();
  };
  const previousRefresh=window.refresh;
  if(typeof previousRefresh==='function')window.refresh=async function(...args){
  ...
  const pendingItems=()=> (state.recurringMonth||[]).filter(x=>x.status==='pending'||x.status==='overdue');
  const basePendingItems=()=>pendingItems().filter(x=>(x.currency||state.base)===state.base);
  function currencyTotal(rows,value=x=>x.amount){
    const totals=new Map();
    (rows||[]).forEach(x=>{const c=x.currency||state.base;totals.set(c,(totals.get(c)||0)+n(value(x)))});
    return [...totals.entries()].filter(([,v])=>Math.abs(v)>1e-9).map(([c,v])=>money(v,c)).join(' · ')||money(0,state.base);
  }
  function dueLabel(x){
    if(!x?.due_date)return '';
    const d=String(x.due_date).slice(0,10),day=Number(d.slice(8,10));
    return Number(x.day_of_month)===31?`Cuối tháng · ${d}`:`Ngày ${day} · ${d}`;
  }
  function intervalLabel(x){const k=Number(x.interval_months||1);return k===1?'Hàng tháng':k===12?'Hàng năm':`Mỗi ${k} tháng`}
  ...
  function extraFixedRecurring(base,rows){
    const grouped=new Map();
    rows.filter(x=>x.cost_type==='fixed').forEach(x=>grouped.set(x.category_id,(grouped.get(x.category_id)||0)+n(x.amount)));
    let plannedTotal=0,uncovered=0;
    const plannedByCategory=new Map();
    activeCategories('expense').filter(c=>c.cost_type==='fixed').forEach(c=>{
      const remaining=Math.max(0,n(c.planned_amount)-n(V.categoryActualBase(c.id,'expense')));
      plannedByCategory.set(c.id,remaining);plannedTotal+=remaining;
    });
    grouped.forEach((amount,id)=>{uncovered+=Math.max(0,amount-(plannedByCategory.get(id)||0))});
    const allocationHeadroom=Math.max(0,n(base.fixedNeed)-plannedTotal);
    return Math.max(0,uncovered-allocationHeadroom);
  }
  function correctedSafeSpend(){
    ...
    const rows=basePendingItems();
    const fixedTotal=rows.filter(x=>x.cost_type==='fixed').reduce((s,x)=>s+n(x.amount),0);
    const variableTotal=rows.filter(x=>x.cost_type!=='fixed').reduce((s,x)=>s+n(x.amount),0);
    const extraFixed=extraFixedRecurring(base,rows);
    const protectedTotal=n(base.protectedTotal)+extraFixed+variableTotal;
    const variableRemaining=Math.max(0,n(base.variableRemaining)-variableTotal);
    const afterProtection=Math.max(0,n(base.spendableBalance)-protectedTotal);
    return {...base,recurringNeed:fixedTotal+variableTotal,extraFixed,variableRecurringNeed:variableTotal,protectedTotal,variableRemaining,available:Math.max(0,Math.min(afterProtection,variableRemaining))};
  }
  ...
  function patchSafeSpend(html){
    ...
    const extra=x.recurringNeed?` Trong đó ${money(x.recurringNeed,state.base)} là khoản định kỳ ${state.base} chưa xác nhận.`:'';
    return html.replace(/(<span class="v7-kicker">CÓ THỂ TIÊU AN TOÀN<\/span>\s*<strong>)[\s\S]*?(<\/strong>\s*<p>)[\s\S]*?(<\/p>)/,`$1${money(x.available,state.base)}$2Sau khi giữ lại ${money(x.protectedTotal,state.base)} cho nghĩa vụ, mục tiêu và dự phòng.${extra}$3`);
  }
  function recurringStrip(){
    const items=pendingItems();if(!items.length)return '';
    const overdue=items.filter(x=>x.status==='overdue');
    return `<section class="rec-strip ${overdue.length?'overdue':''}"><div><span>CHI ĐỊNH KỲ · ${state.month.replace('-','/')}</span><strong>${items.length} khoản chờ · ${currencyTotal(items)}</strong><small>${overdue.length?`${overdue.length} khoản đã quá hạn`:'Chưa trừ tiền cho đến khi bạn xác nhận'}</small></div><div class="rec-strip-actions">${items.slice(0,2).map(x=>`<button data-rec-action="confirm" data-rec-id="${x.id}">${esc(x.name)} · ${money(x.amount,x.currency)}</button>`).join('')}<button class="primary" data-rec-action="manage-month">Xem tất cả</button></div></section>`;
  }
  ...
  function recurringPanel(){
    const rows=state.recurringMonth||[],pending=pendingItems(),expectedRows=rows.filter(x=>x.status!=='skipped'),paidRows=rows.filter(x=>x.status==='paid');
    return `<section class="rec-section"><div class="rec-head"><div><span class="rec-eyebrow">DÒNG TIỀN ĐỊNH KỲ</span><h2>Khoản chờ xác nhận</h2><p>Dự kiến không làm giảm số dư. Chỉ khi xác nhận mới tạo giao dịch thật.</p></div><div class="rec-head-actions"><button data-rec-action="manage">Quản lý</button><button class="primary" data-rec-action="new">＋ Thêm định kỳ</button></div></div><div class="rec-summary"><div><span>Dự kiến tháng</span><strong>${currencyTotal(expectedRows)}</strong></div><div><span>Đã xác nhận</span><strong>${currencyTotal(paidRows,x=>x.actual_amount)}</strong></div><div><span>Còn chờ</span><strong class="${pending.some(x=>x.status==='overdue')?'red':''}">${currencyTotal(pending)}</strong></div></div><div class="rec-grid">${rows.length?rows.map(recurringCard).join(''):'<div class="rec-empty">Chưa có khoản định kỳ trong tháng này. Thêm tiền nhà, Wi‑Fi, điện thoại, bảo hiểm… để app nhắc nhưng không tự trừ.</div>'}</div></section>`;
  }

  const dashboardBefore=V.dashboardV3,budgetBefore=V.budgetV3;
  if(typeof dashboardBefore==='function')V.dashboardV3=()=>{
    let html=patchSafeSpend(dashboardBefore()),strip=recurringStrip();
  ...
  document.addEventListener('click',e=>{
    ...
    const action=b.dataset.recAction,id=b.dataset.recId||'',dlg=document.getElementById('modal');
    if(action==='new'||action==='edit'){
      if(dlg?.open)dlg.close();
      return setTimeout(()=>openRecurring(action==='edit'?id:''),0);
    }
    if(action==='confirm')return openRecurringConfirm(id);
    if(action==='manage'||action==='manage-month')return showRecurringManager();
    if(action==='close')return dlg?.close();
