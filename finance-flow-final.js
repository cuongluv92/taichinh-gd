(() => {
  ...
  function plannedIncomeBasis(){
    ...
    const actual=typeof V.statsFor==='function'&&typeof V.periodTransactions==='function'?Number(V.statsFor(V.periodTransactions('month',state.month))?.income||0):Number(typeof monthTotals==='function'?monthTotals().inc:0);
  ...
  function patchRatios(){
    ...
    if(foot){
      foot.querySelector('.ux-income-pct')?.remove();
